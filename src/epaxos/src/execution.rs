...
impl ExecEngine {
    ...
    fn apply(&mut self, msgs: Vec<(usize, i32)>) {
        unsafe {
            for (replica, instance) in msgs {
                if LOGS[replica][instance as usize].as_mut().unwrap().status == LogStatus::Executed
                {
                    continue;
                }
    ...
    // pub fn run(&mut self) {
    //     println!("exec engine start running");
    //     let mut problem_instance = vec![-1; self.replica_num];
    //     let mut timeout = vec![0; self.replica_num];
    //     loop {
    //         let mut executed = false;
    //         unsafe {
    //             for replica in 0..self.replica_num {
    //                 let instance = EXECUTED_UP_TO[replica];
    //                 match LOGS[replica][instance as usize].as_ref() {
    //                     Some(entry) => {
    //                         if entry.status == LogStatus::Executed {
    //                             EXECUTED_UP_TO[replica] = instance + 1;
    ...
    //     let mut stack: Vec<(usize, i32)> = Vec::new();
    //     stack.push((exec_replica, exec_instance));
    //     // self.scc(&mut index, replica, instance, &mut stack)
    //     // println!(
    //     //     "find scc replica{}  instance{}",
    //     //     exec_replica, exec_instance
    //     // );
    //     unsafe {
    //         while visit >= 0 {
    //             // get
    ...
    //             // println!("executed {}-{}", replica, instance);
    //             if replica == self.id {
    //                 for iter in LOGS[replica][instance as usize]
    //                     .as_ref()
    //                     .unwrap()
